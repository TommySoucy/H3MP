...
namespace H3MP.Tracking
{
    public class TrackedSosig : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            if (sendOnAwake)
            {
                ...
                if (ThreadManager.host)
                {
                    ...
                    Server.AddTrackedSosig(data, 0);
                }
                else
                {
                    // Tell the server we need to add this item to global tracked sosigs
                    data.localWaitingIndex = Client.localSosigCounter++;
                    Client.waitingLocalSosigs.Add(data.localWaitingIndex, data);
                    ClientSend.TrackedSosig(data);
                }
            }
        }
        ...
        {
            if (physicalSosigScript != null && physicalSosigScript.CoreRB != null && data.controller != GameManager.ID && data.position != null && data.rotation != null)
            {
        ...
        private void OnDestroy()
        {
            ...
            GameManager.trackedSosigBySosig.Remove(physicalSosigScript);
            ...
            for(int i=0; i < physicalSosigScript.Links.Count; ++i)
            {
                ...
                {
                    GameManager.EnsureUncontrolled(physicalSosigScript.Links[i].O);
                }
            }
                    ...
                    if (data.controller == GameManager.ID)
                    {
                        ...
                        else // We have a potential new host to give control to
                        {
                            ...
                            if (data.trackedID > -1)
                            {
                                ...
                                if (ThreadManager.host)
                                {
                                    ServerSend.GiveSosigControl(data.trackedID, otherPlayer, new List<int>() { GameManager.ID });
                                }
                                else
                                {
                                    ClientSend.GiveSosigControl(data.trackedID, otherPlayer, new List<int>() { GameManager.ID });
                                }
        ...
        private void DestroyGlobally(ref bool removeFromLocal)
        {
            ...
            if (data.trackedID > -1)
            {
                ...
                if (sendDestroy)
                {
                    ...
                    if (ThreadManager.host)
                    {
                        ServerSend.DestroySosig(data.trackedID, data.removeFromListOnDestroy);
                    }
                    else
                    {
                        ClientSend.DestroySosig(data.trackedID, data.removeFromListOnDestroy);
                    }
                }
                ...
                if (data.removeFromListOnDestroy)
                {
                    if (ThreadManager.host)
                    {
                        Server.sosigs[data.trackedID] = null;
                        Server.availableSosigIndices.Add(data.trackedID);
                    }
                    else
                    {
                        Client.sosigs[data.trackedID] = null;
                    }

                    GameManager.sosigsByInstanceByScene[data.scene][data.instance].Remove(data.trackedID);
                }
            }
            else // trackedID == -1, note that it cannot == -2 because DestroyGlobally will never get called in that case due to skipDestroyProcessing flag
            {
