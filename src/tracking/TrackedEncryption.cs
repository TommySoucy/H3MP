...
{
    public class TrackedEncryption : MonoBehaviour
    {
        public TNH_EncryptionTarget physicalEncryptionScript;
        public TrackedEncryptionData data;
        public bool awoken;
        ...
        private void Awake()
        {
            ...
            if (sendOnAwake)
            {
                ...
                if (ThreadManager.host)
                {
                    ...
                    Server.AddTrackedEncryption(data, 0);
                }
                else
                {
                    // Tell the server we need to add this item to global tracked Encryptions
                    data.localWaitingIndex = Client.localEncryptionCounter++;
                    Client.waitingLocalEncryptions.Add(data.localWaitingIndex, data);
                    ClientSend.TrackedEncryption(data);
                }
            }
        ...
        private void OnDestroy()
        {
            ...
            if(data.controller != GameManager.ID && physicalEncryptionScript.UsesRegenerativeSubTarg)
            {
            ...
            GameManager.trackedEncryptionByEncryption.Remove(physicalEncryptionScript);
                    ...
                    if (data.controller == GameManager.ID)
                    {
                        ...
                        else // We have a potential new host to give control to
                        {
                            ...
                            if (data.trackedID > -1)
                            {
                                ...
                                if (ThreadManager.host)
                                {
                                    ServerSend.GiveEncryptionControl(data.trackedID, otherPlayer, new List<int>() { GameManager.ID });
                                }
                                else
                                {
                                    ClientSend.GiveEncryptionControl(data.trackedID, otherPlayer, new List<int>() { GameManager.ID });
                                }
        ...
        private void DestroyGlobally(ref bool removeFromLocal)
        {
            ...
            if (data.trackedID > -1)
            {
                ...
                if (sendDestroy)
                {
                    ...
                    if (ThreadManager.host)
                    {
                        ServerSend.DestroyEncryption(data.trackedID, data.removeFromListOnDestroy);
                    }
                    else
                    {
                        ClientSend.DestroyEncryption(data.trackedID, data.removeFromListOnDestroy);
                    }
                }
                ...
                if (data.removeFromListOnDestroy)
                {
                    if (ThreadManager.host)
                    {
                        Server.encryptions[data.trackedID] = null;
                        Server.availableEncryptionIndices.Add(data.trackedID);
                    }
                    else
                    {
                        Client.encryptions[data.trackedID] = null;
                    }

                    GameManager.encryptionsByInstanceByScene[data.scene][data.instance].Remove(data.trackedID);
                }
            }
            else // trackedID == -1, note that it cannot == -2 because DestroyGlobally will never get called in that case due to skipDestroyProcessing flag
            {
