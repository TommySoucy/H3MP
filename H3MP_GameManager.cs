...
namespace H3MP
{
    public class H3MP_GameManager : MonoBehaviour
    {
        ...
        public static void Reset()
        {
            ...
            H3MP_TrackedItem.unknownParentWaitList.Clear();
        ...
        private static H3MP_TrackedSosig MakeSosigTracked(Sosig sosigScript)
        {
            ...
            else
            {
                ...
                if(trackedItem == null)
                {
                    H3MP_TrackedItem.unknownSosigInventoryObjects.Add(sosigScript.Hand_Secondary.HeldObject, new KeyValuePair<H3MP_TrackedSosigData, int>(data, 1));
                    data.inventory[1] = -1;
                }
                else
                {
            ...
            for(int i=0; i < sosigScript.Inventory.Slots.Count; ++i)
            {
                ...
                else
                {
                    ...
                    if (trackedItem == null)
                    {
                        H3MP_TrackedItem.unknownSosigInventoryObjects.Add(sosigScript.Inventory.Slots[i].HeldObject, new KeyValuePair<H3MP_TrackedSosigData, int>(data, i + 2));
                        data.inventory[i + 2] = -1;
                    }
                    else
                    {
                    ...
                    if (H3MP_Server.items[i] != null && H3MP_Server.items[i].controller == clientID)
                    {
                        ...
                        if (destroyed) // No other player to take control, destroy
                        {
                            if(trackedItem.physicalItem == null)
                            {
                                ...
                                if(clientID == H3MP_GameManager.ID)
                                {
