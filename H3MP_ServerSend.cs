...
namespace H3MP
{
    internal class H3MP_ServerSend
    {
        ...
        public static void AttachableFirearmFire(int clientID, int trackedID, bool firedFromInterface, List<Vector3> positions, List<Vector3> directions)
        {
        ...
        public static void InitTNHInstances(int toClientID)
        {
            using (H3MP_Packet packet = new H3MP_Packet((int)ServerPackets.initTNHInstances))
            {
                ...
                else
                {
                    ...
                    {
                        packet.Write(TNHInstanceEntry.Key);
                        packet.Write(TNHInstanceEntry.Value.controller);
                        if (TNHInstanceEntry.Value.playerIDs == null || TNHInstanceEntry.Value.playerIDs.Count == 0)
                        {
                            packet.Write(0);
                        }
                        else
                        {
                            packet.Write(TNHInstanceEntry.Value.playerIDs.Count);
                            foreach(int playerID in TNHInstanceEntry.Value.playerIDs)
                            {
                                packet.Write(playerID);
                            }
                        }
                        if (TNHInstanceEntry.Value.currentlyPlaying == null || TNHInstanceEntry.Value.currentlyPlaying.Count == 0)
                        {
                            packet.Write(0);
                        }
                        else
                        {
                            packet.Write(TNHInstanceEntry.Value.currentlyPlaying.Count);
                            foreach(int playerID in TNHInstanceEntry.Value.currentlyPlaying)
                            {
                                packet.Write(playerID);
                            }
                        }
                        if (TNHInstanceEntry.Value.played == null || TNHInstanceEntry.Value.played.Count == 0)
                        {
                            packet.Write(0);
                        }
                        else
                        {
                            packet.Write(TNHInstanceEntry.Value.played.Count);
                            foreach(int playerID in TNHInstanceEntry.Value.played)
                            {
                                packet.Write(playerID);
                            }
                        }
                        if (TNHInstanceEntry.Value.dead == null || TNHInstanceEntry.Value.dead.Count == 0)
                        {
                            packet.Write(0);
                        }
                        else
                        {
                            packet.Write(TNHInstanceEntry.Value.dead.Count);
                            foreach(int playerID in TNHInstanceEntry.Value.dead)
                            {
                                packet.Write(playerID);
                            }
                        }
                        packet.Write(TNHInstanceEntry.Value.tokenCount);
                        packet.Write(TNHInstanceEntry.Value.holdOngoing);
                        packet.Write(TNHInstanceEntry.Value.curHoldIndex);
                        packet.Write(TNHInstanceEntry.Value.level);
                        packet.Write((short)TNHInstanceEntry.Value.phase);
                        if (TNHInstanceEntry.Value.activeSupplyPointIndices == null || TNHInstanceEntry.Value.activeSupplyPointIndices.Count == 0)
                        {
                            packet.Write(0);
                        }
                        else
                        {
                            packet.Write(TNHInstanceEntry.Value.activeSupplyPointIndices.Count);
                        ...
                        {
                            packet.Write(TNHInstanceEntry.Value.raisedBarriers.Count);
                            foreach (int index in TNHInstanceEntry.Value.raisedBarriers)
                            {
                                packet.Write(index);
                            }
                        }
                        if (TNHInstanceEntry.Value.raisedBarrierPrefabIndices == null || TNHInstanceEntry.Value.raisedBarrierPrefabIndices.Count == 0)
                        {
                            packet.Write(0);
                        }
                        else
                        {
                            packet.Write(TNHInstanceEntry.Value.raisedBarrierPrefabIndices.Count);
                            foreach (int prefabIndex in TNHInstanceEntry.Value.raisedBarrierPrefabIndices)
                            {
                                packet.Write(prefabIndex);
                            }
                        }
                        packet.Write(TNHInstanceEntry.Value.letPeopleJoin);
                        packet.Write(TNHInstanceEntry.Value.progressionTypeSetting);
                        packet.Write(TNHInstanceEntry.Value.healthModeSetting);
                        packet.Write(TNHInstanceEntry.Value.equipmentModeSetting);
                        packet.Write(TNHInstanceEntry.Value.targetModeSetting);
                        packet.Write(TNHInstanceEntry.Value.AIDifficultyModifier);
                        packet.Write(TNHInstanceEntry.Value.radarModeModifier);
                        packet.Write(TNHInstanceEntry.Value.itemSpawnerMode);
                        packet.Write(TNHInstanceEntry.Value.backpackMode);
